...
class csv_to_excel:
    def __init__(self, root):
        ...
        self.message_label2 = Label(self.f, 
                                    text = 'Converter of CSV to Execel file', 
...
def convert_csv_to_xls(self):
    try:
        ...
        if(len(df) == 0):
            ...
            with pd.ExcelWriter('MyExample.xls')
                    df.to_excel(writer, 'MESheet')
                    writer.save()
...
def display_xls_file(self):
    try:
        self.file_name = filedialog.filedialog.askopenfilename(initialdir = '/Desktop',
...
root = Tk()
root.title('ME ---Convert CSV to Excel File')
